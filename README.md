...
| `items`             | `JSX.Element[]`                                                                     | The array of items to be displayed in the carousel.                                                                                     | `[]`                                   |
...
| `width`             | `string \| number`                                                                  | Width of each item; proportional to container width if a number is provided.                                                            | `'400px'`                              |
| `height`            | `string \| number`                                                                  | Height of each item; proportional to container height if a number is provided.                                                          | `'300px'`                              |
...
| `perspective`       | `string`                                                                            | [CSS Perspective](https://developer.mozilla.org/en-US/docs/Web/CSS/perspective) distance for the 3D effect.                             | `'auto'`                               |
...
| `defaultOption`     | `DefaultOption`                                                                     | Configuration for default layout, including numOfSlides, widthFactor, depthFactor, and angleFactor (see table below).                   | `{}`                                   |
...
| `pauseOnHover`      | `boolean`                                                                           | If true, pauses autoplay when hovered.                                                                                                  | `true`                                 |
| `onChange`          | `(index: number, item: JSX.Element) => void`                                        | Callback function triggered when the centered item changes.                                                                             | `undefined`                            |
| `onClickItem`       | `(e: MouseEvent, index: number, item: JSX.Elemnt, isCurrentIndex: boolean) => void` | Callback when an item is clicked, providing event, index, item, and if it’s the current item.                                           | `undefined`                            |
| `autoPlay`          | `boolean`                                                                           | Enables autoplay of the carousel.                                                                                                       | `true`                                 |
| `interval`          | `number`                                                                            | Interval time (in ms) for autoplay.                                                                                                     | `3000`                                 |
| `infiniteLoop`      | `boolean`                                                                           | Enables infinite loop of the carousel items.                                                                                            | `true`                                 |
| `autoFocus`         | `boolean`                                                                           | If true, the carousel will automatically focus on the first item.                                                                       | `true`                                 |
...
The `default` layout uses the same structure. [See Example](https://github.com/Byongho96/react-responsive-3d-carousel/blob/main/src/utils/getCarouselIndex.ts).
...
export type LayoutInfo = {
  width: number | string // proportional to container width if a number. When it's 'auto' the width prop is used.
  height: number | string // proportional to container height if a number. When it's 'auto' the height prop is used.
  translate: {
    x: number | string // proportional to container width if a number
    y: number | string // proportional to container height if a number
    z: number | string // proportional to container width if a number
  }
  rotation: {
    x: number // in degrees
    y: number // in degrees
  ...
  offset: {
    x: number | string // proportional to item width if a number
    y: number | string // proportional to item height if a number
    z: number | string // proportional to item width if a number
  }
}
```
...
| `width`         | `string`                                 | Width of each indicator.                                                                                                  | `'0.6rem'`                              |
| `height`        | `string`                                 | Height of each indicator.                                                                                                 | `'0.6rem'`                              |
...
- The `*-container` class is used to position the carousel component. It is styled with `position: absolute` and has `pointer-events: none` to prevent user interaction above the carousel items.
