...
<Carousel items={items} pauseOnHover={false} showArrows={false} showIndicators={false} />

:::info

To use the `slideNext` and `slidePrev` functions on carousel item clicks, ensure that the `pauseOnHover` prop is set to false.
...
import CarouselItemWithContext from '@site/src/components/CarouselItemWithContext';
